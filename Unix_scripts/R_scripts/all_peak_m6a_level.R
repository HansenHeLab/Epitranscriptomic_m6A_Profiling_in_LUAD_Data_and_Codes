...
peak_bed <- read.table(peak_file)

######################################
input <-read.table(input_file);

# for input 
reg_u <- sort(unique(input$V5));
cnt <- length(reg_u)
cov_input_list <- list()
...
out <- cbind(peak_bed, summit_pos, summit_pos+1, summit_fd,  reg_u, strand, peak_ip_pileup, peak_input_pileup, peak_fd, peak_ip_pileup_sum, peak_input_pileup_sum, peak_sum_fd);
